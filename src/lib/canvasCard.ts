...
export async function generateQuizCard(
  ...
  drawGradientBackground(ctx, width, height, DEFAULT_STYLE);
  
  // Logo / Brand
  drawText(ctx, '$HORNY', width / 2, 50, 32, DEFAULT_STYLE.accentColor);
  
  // Class title
  drawText(ctx, className.toUpperCase(), width / 2, 120, 42, DEFAULT_STYLE.textColor);
  ...
  gradient.addColorStop(0, DEFAULT_STYLE.gradientStart);
  gradient.addColorStop(1, DEFAULT_STYLE.gradientEnd);
  ctx.fillStyle = gradient;
  ctx.fillRect(meterX, meterY, meterWidth * (level / 100), meterHeight);
  
  // Meter border
  ctx.strokeStyle = DEFAULT_STYLE.accentColor;
  ctx.lineWidth = 2;
  ctx.strokeRect(meterX, meterY, meterWidth, meterHeight);
  
  // Level text
  drawText(ctx, `HORNY LEVEL: ${level}%`, width / 2, 230, 24, DEFAULT_STYLE.accentColor);
  ...
  // Footer
  drawText(ctx, 'hornymeta.io', width / 2, height - 30, 16, '#888888');
