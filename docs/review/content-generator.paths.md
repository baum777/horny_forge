...
- `vite.config.ts` → Vite dev proxy for `/api` + `/base_assets`.

## Path Finder (key files)
| Path | Description |
| --- | --- |
| `src/components/MetaForge/MetaForge.tsx` | Main Content Forge UI container (state, forge/release/share). |
| `src/components/MetaForge/BasePicker.tsx` | Fetches `/api/content-pool` and selects base template. |
...
| `server/src/app.ts` | Express app wiring `/api/forge`, `/api/content-pool`, gamification, etc. |
...
| `server/src/routes/assetPool.ts` | Returns base templates from `server/public/base_assets`. |
| `server/public/base_assets/*` | Base content template images. |
...
| `docs/assets.md` | Base image serving via `/base_assets`. |
| `vite.config.ts` | Proxy for `/api` + `/base_assets` to backend. |
